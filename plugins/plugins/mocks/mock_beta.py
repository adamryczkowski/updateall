...
Different visual styling than Mock Alpha for easy differentiation.
...
class MockBetaPlugin(BasePlugin):
    ...
    Different parameters than Mock Alpha for varied load testing.

    Attributes:
        name: Plugin identifier ("mock-beta")
        command: Base command used ("bash")
        description: Human-readable description

    Example:
        >>> plugin = MockBetaPlugin()
        >>> plugin.name
        'mock-beta'
    ...
    def description(self) -> str:
        ...
        return "Mock Beta - Multi-phase update with real CPU/network load (debugging)"
    ...
    def get_interactive_command(self, dry_run: bool = False) -> list[str]:
        ...
        script = f"""
{scripts_dir}/mock_check.sh mock-beta {dry_run_arg}
{scripts_dir}/mock_download.sh mock-beta {dry_run_arg}
{scripts_dir}/mock_execute.sh mock-beta {dry_run_arg}
"""
    ...
    def get_phase_commands(self, dry_run: bool = False) -> dict[Phase, list[str]]:
        ...
        Mock Beta phases use external scripts that generate real load:
